...
class LargeModelService(Node):
    ...
    def init_camera_device(self):
        ...
        if self.camera_type_param == 'usb':
            if self._init_usb_camera():
                ...
                # 如果USB摄像头初始化失败，尝试自动检测并使用CSI摄像头作为备选
                self.get_logger().info("Trying to initialize CSI camera as fallback...")
            ...
            if self._init_csi_camera():
                ...
                # 如果CSI摄像头初始化失败，尝试自动检测并使用USB摄像头作为备选
                self.get_logger().info("Trying to initialize USB camera as fallback...")
                if self._init_usb_camera():
                    return True
                return False
        else:
            self.get_logger().error(f"Invalid camera type specified: {self.camera_type_param}. Valid options are: 'csi', 'usb'")
            # 如果摄像头类型无效，尝试自动检测并初始化可用的摄像头
            self.get_logger().info("Trying to automatically detect and initialize available camera...")
            if self._init_csi_camera():
                return True
            if self._init_usb_camera():
                return True
    ...
    def _detect_usb_camera_device(self):
        ...
        try:
            ...
            # 如果还是没有找到，尝试使用第一个可用的video设备作为备选/If still not found, try to use the first available video device as fallback
            for device in video_devices:
                if device.startswith('video'):
                    try:
                        ...
                        if device_number:
                            device_id = int(device_number.group(1))
                            self.get_logger().warn(f"No USB camera found, using first available video device: /dev/{device} (ID: {device_id}) as fallback")
                            return device_id
                    except Exception as e:
                        continue
    ...
    def _init_usb_camera(self):
        ...
        try:
            ...
            if cap.isOpened():
                # 设置低分辨率以减少资源使用/Set low resolution to reduce resource usage
                # 注意：使用更低的分辨率以避免磁盘空间和内存问题
                self.get_logger().info("Setting low resolution to minimize resource usage...")
                cap.set(cv2.CAP_PROP_FRAME_WIDTH, 160)  # 极低分辨率
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 120)  # 极低分辨率
                cap.set(cv2.CAP_PROP_FPS, 10)  # 极低帧率
                
                # 获取实际设置/Get actual settings
                actual_width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
                actual_height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
                actual_fps = cap.get(cv2.CAP_PROP_FPS)
                self.get_logger().info(f"Actual camera settings: {actual_width}x{actual_height} @ {actual_fps:.1f}fps")
                
                # 等待初始化/Wait for initialization
                self.get_logger().info("Waiting 3 seconds for camera initialization...")
                time.sleep(3)
                
                # 尝试多次读取帧来验证摄像头是否真正工作/Try reading multiple frames to verify that the camera is actually working
                self.get_logger().info("Waiting for USB camera frame...")
                frame_read_success = False
                for i in range(5):  # More retries for USB camera
                    try:
                        ret, frame = cap.read()
                        if ret and frame is not None and frame.size > 0:
                            self.get_logger().info(f"USB Camera initialized successfully at {actual_width}x{actual_height}. Frame shape: {frame.shape}")
                            self.cap = cap
                            self.camera_initialized = True
                            self.camera_type = "usb"
                            return True
                        else:
                            self.get_logger().warn(f"USB Camera Attempt {i+1}/5: Failed to read frame, retrying...")
                            time.sleep(1)  # Longer delay between retries
                    except Exception as e:
                        self.get_logger().warn(f"Exception during frame read {i+1}/5: {e}")
                        time.sleep(1)
                
                # 如果所有尝试都失败，检查磁盘空间/If all attempts fail, check disk space
                if not frame_read_success:
                    ...
                    if free_space_gb < 5:
                        self.get_logger().error(f"CRITICAL: Low disk space ({free_space_gb:.1f} GB available). This may be causing camera frame capture failures.")
                        self.get_logger().error("Please free up disk space and try again.")
                
                # 如果所有尝试都失败/If all attempts fail
                self.get_logger().warn("USB Camera opened but failed to read frame after multiple attempts.")
                cap.release()
            else:
                self.get_logger().warn("Failed to open USB camera.")
                
            # 如果所有尝试都失败，使用模拟摄像头作为备选/If all else fails, use mock camera as fallback
            self.get_logger().info("Falling back to mock camera mode...")
            self.camera_initialized = True
            self.camera_type = "mock"
            self.get_logger().warn("Using mock camera mode - no actual camera feed available but system will continue to function")
            return True
            ...
            # 即使出错也返回True，使用模拟摄像头模式/Even if error, return True to use mock camera mode
            self.get_logger().info("Falling back to mock camera mode due to exception...")
            self.camera_initialized = True
            self.camera_type = "mock"
            self.get_logger().warn("Using mock camera mode - no actual camera feed available but system will continue to function")
            return True
    ...
    def _safe_play_audio(self, text_to_speak: str):
        ...
        if not self.text_chat_mode and text_to_speak:
            try:
                ...
                result = self.model_client.voice_synthesis(text_to_speak, self.tts_out_path)
                # Only play audio if synthesis was successful
                if result == 0:
